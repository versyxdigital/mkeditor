...
| Area                      | Decision                                                                                                                                                                                                                                                                                                                                           |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Provider abstraction      | [Vercel AI SDK](https://sdk.vercel.ai) (`ai` + `@ai-sdk/openai` + `@ai-sdk/anthropic` + `ollama-ai-provider`). One `streamText`/`generateText` surface, one tool-call shape, one message schema across all three providers. Adding a fourth provider later is one adapter and a settings row.                                                      |
...
| #   | Phase                                                              | Status |
| --- | ------------------------------------------------------------------ | ------ |
| 1   | Main-process infrastructure (AppAssistant + keystore + IPC)        | 🔵     |
...
1. **Install deps**: `ai`, `@ai-sdk/openai`, `@ai-sdk/anthropic`, `ollama-ai-provider`, `zod`. SDK only used in `src/app/` for now; webpack `externals` keeps it out of the renderer bundle.
...
4. **`src/app/lib/AppAssistant.ts`** — owns the Vercel AI SDK clients. Public surface:
   - `chat(callId, request)` — starts a `streamText` for the given provider/model/messages/tools. Forwards text deltas as `from:ai:chunk { callId, text }`, tool-call deltas as `from:ai:tool-call { callId, tool }`, completion as `from:ai:done { callId, usage }`, errors as `from:ai:error { callId, message }`.
   - `cancel(callId)` — abort the in-flight `AbortController`.
   - `submitToolResult(callId, toolCallId, result)` — feeds the SDK's tool-result loop.
   - `listOllamaModels(baseUrl)` — `GET {baseUrl}/api/tags` → array of model names.
5. **`src/app/AppBridge.ts`** — register `ipcMain.on('to:ai:chat' | 'to:ai:cancel' | 'to:ai:tool-result' | 'to:ai:config:get' | 'to:ai:config:set' | 'to:ai:key:set' | 'to:ai:key:clear' | 'to:ai:ollama:list')`. `to:ai:config:get` responds via a `from:ai:config` push (sanitized — `hasKey: boolean`, never the key value).
