...
- 🔵 **Custom title bar (VSCode-style).** Frameless window on Windows/Linux with an in-window logo + File/Edit/View/Help menu + native min/max/close. macOS keeps its native menu bar via `titleBarStyle: 'hiddenInset'`. Three-phase plan in [TITLE_BAR.md](TITLE_BAR.md).
