...
export const PreviewPane: React.FC = () => {
  const { mode, editorManager, dispatcher } = useManagers();
  ...
  const baseInfoRef = React.useRef({ mode, activeFile, treeRoot });
  React.useEffect(() => {
    baseInfoRef.current = { mode, activeFile, treeRoot };
  }, [mode, activeFile, treeRoot]);
  ...
  React.useEffect(() => {
    dispatcher.render();
  }, [activeFile, treeRoot, dispatcher]);
...
function buildPreviewFragment(
  ...
  // Prefer the active file's directory so `![](sibling.png)` works
  // the way CommonMark prescribes; fall back to the workspace root
  // so a freshly-mounted preview without an open file still resolves
  // image paths to something useful.
  const baseDir = ctx.activeFile ? dirnameOf(ctx.activeFile) : ctx.treeRoot;
